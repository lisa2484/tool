...
if (isset($_POST["sql"])) $sql = trim($_POST["sql"]);
if (isset($_POST["page"])) $page = empty($_POST["page"]) ? 1 : trim($_POST["page"]);
$limit = ($page - 1) * 1000;
$err = "";
$req = "";
$str = "";
if (!empty($sql)) {
    $link_ID = mysqli_connect('127.0.0.1', 'sa', 'abc123');
    switch ($type) {
        case "show":
...
function SqlQueryArray($QSqlStr, $dbLink)
{  // 查詢資料匯入(這樣就不必一直與資料庫連線取得清單資料)
    $QueryStr = trim($QSqlStr);
    $DataAry = null;
    if ($QueryStr != "") {
        $ExeQuery = mysqli_query($dbLink, $QueryStr);
        while ($reData = mysqli_fetch_assoc($ExeQuery)) {
            $DataAry[] = $reData;
        }
    }
    return $DataAry;
